...
from ODNN_functions import generate_complex_weights,generate_fields_ts,create_labels
from odnn_generate_label import compute_label_centers, compose_labels_from_patterns, generate_detector_patterns
...
if torch.cuda.is_available():
    device = torch.device('cuda:6')           # 或者 'cuda:0'
    print('Using Device:', device)
else:
...
TRAIN_NUM_SAMPLES = 10000  # default number of training samples
TEST_NUM_SAMPLES = 1000   # fixed test set size for evaluation
TEST_SET_SEED = 13579     # seed to keep test set deterministic across runs
num_data = TRAIN_NUM_SAMPLES # options: 1. random datas 2.eigenmodes
num_modes = 6 #the mode number of MMF 3 6 10
focus_radius = 5 #4,15 #the size of detection regions(circle)
detectsize = 10  # keep identical geometry to circular label layout
circle_radius = int(round(focus_radius))
if circle_radius <= 0:
    raise ValueError("focus_radius must be positive for circle detector layout.")

pattern_size = circle_radius * 2
if pattern_size % 2 == 0:
    pattern_size += 1  # ensure odd size so pattern centers align with grid

detector_pattern_stack = generate_detector_patterns(
    pattern_size,
    pattern_size,
    num_modes,
    shape="circle",
)
detector_centers, _, _ = compute_label_centers(
    layer_size,
    layer_size,
    num_modes,
    circle_radius,
...
def build_evaluation_regions_from_centers(
    ...
    """Create square evaluation boxes centered on provided coordinates."""
    half_size = detectsize // 2
    regions = []
    preview = np.zeros((height, width), dtype=float)

    for cy, cx in centers:
        x_start = max(cx - half_size, 0)
        x_end = min(cx + half_size, width)
        y_start = max(cy - half_size, 0)
        y_end = min(cy + half_size, height)
        regions.append((x_start, x_end, y_start, y_end))
        preview[y_start:y_end, x_start:x_end] = 0.5
        preview[cy, cx] = 1.0

    if visualize:
        plt.figure(figsize=(6, 6))
        plt.imshow(preview, cmap="gray")
        plt.title("Evaluation Regions Preview")
        plt.axis("off")
        plt.show()

    return regions
detector_layout_stack = torch.stack(
    [
        torch.from_numpy(
            compose_labels_from_patterns(
                layer_size,
                layer_size,
                detector_pattern_stack,
                detector_centers,
                Index=i + 1,
                visualize=False,
            )
...
#phase_option 3: (0,pi,...,2pi)  -> random superposition
#phase_option 4: eigenmodes
#phase_option 5: (0,pi,...,pi)


def generate_weights_with_seed(num_samples: int, seed: int):
    """Generate amplitudes/phases with a temporary RNG seed so other draws stay untouched."""
    state = np.random.get_state()
    np.random.seed(seed)
    try:
        amps, phs = generate_complex_weights(num_samples, num_modes, phase_option)
    finally:
        np.random.set_state(state)
    return amps, phs


def build_label_tensor(amplitudes_phases_array: np.ndarray, pred_case_value: int):
    """Create label tensor (N x 1 x layer_size x layer_size) from amplitudes/phases."""
    num_samples = amplitudes_phases_array.shape[0]
    labels = torch.zeros([num_samples, 1, layer_size, layer_size])
    extra = None
    num_detector = None

    if pred_case_value == 1:
        num_detector = detector_layout_stack.shape[2]
        mmf_label = detector_layout_stack
        weights = torch.from_numpy(amplitudes_phases_array[:, :num_detector]).float()
        for idx in range(num_samples):
            labels[idx, 0] = torch.tensordot(weights[idx], mmf_label, dims=([0], [2]))

    if pred_case_value == 2:
        num_detector = num_modes - 1
        mmf_label = detector_layout_stack[:, :, :num_detector]
        weights = torch.from_numpy(amplitudes_phases_array[:, -(num_modes - 1) :]).float()
        for idx in range(num_samples):
            labels[idx, 0] = torch.tensordot(weights[idx], mmf_label, dims=([0], [2]))

    if pred_case_value == 4:
        num_detector = detector_layout_stack.shape[2]
        extra_energy_region = np.sqrt(
            1
            - np.sum(
                (amplitudes_phases_array[:, -2:] * (2 / np.sqrt(5))) ** 2,
                axis=1,
                keepdims=True,
            )
        )
        extra = np.hstack(
            (
                amplitudes_phases_array[:, :3],
                amplitudes_phases_array[:, -2:] * (2 / np.sqrt(5)),
                extra_energy_region,
            )
        )
        mmf_label = detector_layout_stack
        weights = torch.from_numpy(extra).float()
        for idx in range(num_samples):
            labels[idx, 0] = torch.tensordot(weights[idx], mmf_label, dims=([0], [2]))

    if num_detector is None:
        raise ValueError(f"Unsupported pred_case={pred_case_value} in build_label_tensor.")

    return labels, extra, num_detector
...
complex_weights = (amplitudes * np.exp(1j * phases)).astype(np.complex64)


MMF_data_ts = torch.from_numpy(MMF_data)
complex_weights_ts = torch.from_numpy(complex_weights)
image_data_raw = generate_fields_ts(complex_weights_ts, MMF_data_ts, num_data, num_modes, field_size)
image_data = torch.abs(image_data_raw).to(torch.complex64)
...
pred_case = 1
label_size = 100 # 300
label_data, amplitudes_phases_extra, num_detector = build_label_tensor(amplitudes_phases, pred_case)
...
#%% Define test dataset
# Fixed 1000-sample superposition dataset for evaluation.
num_test_data = TEST_NUM_SAMPLES
amplitudes_test, phases_test = generate_weights_with_seed(num_test_data, TEST_SET_SEED)
amplitudes_phases_test_ori = np.hstack((amplitudes_test[:, :], phases_test[:, 1:]))
amplitudes_phases_test = np.hstack((amplitudes_test[:, :], phases_test[:, 1:] / (2 * np.pi)))

complex_weights_test = (amplitudes_test * np.exp(1j * phases_test)).astype(np.complex64)
complex_weights_test_ts = torch.from_numpy(complex_weights_test)
image_test_data_raw = generate_fields_ts(
    complex_weights_test_ts, MMF_data_ts, num_test_data, num_modes, field_size
)
image_test_data = torch.abs(image_test_data_raw).to(torch.complex64)

label_test_data, amplitudes_phases_test_extra, num_detector_test = build_label_tensor(
    amplitudes_phases_test, pred_case
)
if num_detector_test != num_detector:
    raise ValueError("Train/test detection region mismatch.")
...
centers_for_eval = detector_centers[:num_detector]
...
#layer层数:目前是3层 
num_layer_option = [10]   #, 3]#, 4]  # Define the different layer-number ODNN
#num_layer_option = [1]
...
for idx, num_layer in enumerate(num_layer_option):
    ...
    if pred_case == 1: # only amplitude weights
        ...
        target_amp = amplitudes_phases_test[:N, :num_modes]
        current_phase_weights_pred = phases_test[:N, :]
        ...
        amplitudes_diff = np.abs(amplitudes_phases_test[:, :3] - current_amplitude_weights_pred) # amp. weights error
        average_amplitudes_diff = np.mean(amplitudes_diff)
        phases_diff = np.abs(amplitudes_phases_test[:, -2:] - current_weights_pred[:, -2:]) # phase weights error
        ...
        if amplitudes_phases_test_extra is None:
            raise RuntimeError("Expected amplitudes_phases_test_extra for pred_case 4.")
        amplitudes_diff = np.abs(
            amplitudes_phases_test_extra[:num_test_data, :3] - current_amplitude_weights_pred
        )  # amp. weights error
        average_amplitudes_diff = np.mean(amplitudes_diff)
        ...
        phases_diff = np.abs(
            amplitudes_phases_test_ori[:num_test_data, -2:] - current_phase_weights_pred[:, 1:]
        )  # phase weights error
        average_phases_diff = np.mean(phases_diff)
    ...
    for i in range(len(image_test_data)):
        ...
        corr_real = np.corrcoef(
            np.real(complex_weights_test[i]), np.real(complex_weights_pred[i])
        )[0, 1]
        cc_real.append(corr_real)
        corr_imag = np.corrcoef(
            np.imag(complex_weights_test[i]), np.imag(complex_weights_pred[i])
        )[0, 1]
...
##!!3个点
    ...
    N = len(current_amplitude_weights_pred)
    target_amp = l2_normalize_rows(amplitudes_phases_test[:N, :num_modes])
    current_phase_weights_pred = phases_test[:N, :]
    current_amplitude_weights_pred = l2_normalize_rows(current_amplitude_weights_pred)
    amplitudes_diff = np.abs(target_amp - current_amplitude_weights_pred)
...
# # === 保存示例样本 (5 个) 的输入/输出图像及 MAT 数据 ===
# num_visual = min(5, int(image_test_data.shape[0]))
# if num_visual > 0:
#     sample_dir = Path("verify_outputs/super_test_samples")
...
#     phase0_fields = image_test_data[:num_visual, 0].detach().clone()
#     padded_inputs = torch.stack(
#         [
#             complex_pad_asymm(field, pt, pb, pl, pr).unsqueeze(0)
...
#     with torch.no_grad():
#         preds_samples = D2NN(
#             padded_inputs.to(device, dtype=torch.complex64, non_blocking=True)
#         )
#     preds_np = preds_samples.squeeze(1).detach().cpu().numpy()

#     inputs_phase0_np = phase0_fields.cpu().numpy()
#     labels_np = label_test_data[:num_visual, 0].detach().cpu().numpy()
#     raw_small_np = image_data_raw[:num_visual, 0].detach().cpu().numpy()

#     for idx in range(num_visual):
#         raw_small = raw_small_np[idx]
#         raw_tensor = torch.from_numpy(raw_small).to(torch.complex64)
#         raw_padded = complex_pad_asymm(raw_tensor, pt, pb, pl, pr).detach().cpu().numpy()
#         raw_amp = np.abs(raw_padded)

#         phase0_field = inputs_phase0_np[idx]
#         phase0_amp = np.abs(phase0_field)

#         pred_map = preds_np[idx]
#         label_map = labels_np[idx]
#         diff_map = pred_map - label_map

#         sig_masks, _, centers = _masks_from_eval_regions(
#             pred_map.shape, evaluation_regions, detectsize, center_offset=(0, 0)
#         )
#         region_sums_pred = np.array([float(pred_map[m].sum()) for m in sig_masks], dtype=np.float64)
#         amp_pred = np.clip(region_sums_pred, a_min=0.0, a_max=None)
#         amp_pred_norm = amp_pred / (np.linalg.norm(amp_pred) + 1e-12)
#         amp_pred_share = amp_pred_norm ** 2

#         if "amplitudes_phases_test" in globals():
#             label_amp_source = amplitudes_phases_test
#         elif "amplitudes_phases" in globals():
#             label_amp_source = amplitudes_phases
#         else:
#             raise RuntimeError("Cannot locate label amplitudes array for visualization output.")

#         amp_label = np.asarray(label_amp_source[idx, :num_detector], dtype=np.float64)
#         amp_label_norm = amp_label  # 标签幅值已按照数据生成流程归一化
#         amp_label_share = amp_label_norm ** 2

#         fig, axes = plt.subplots(1, 5, figsize=(18, 3.6))
#         imgs = [
#             (raw_amp, "Input (orig phase)"),
#             (phase0_amp, "Input (phase=0)"),
#             (pred_map, "Model output"),
#             (label_map, "Label"),
#             (diff_map, "Output - Label"),
#         ]

#         for ax, (img, title) in zip(axes, imgs):
#             if title == "Output - Label":
#                 clim = np.max(np.abs(img)) or 1.0
#                 im = ax.imshow(img, cmap="bwr", vmin=-clim, vmax=clim)
#             else:
...
#         fig.tight_layout()
#         fig.savefig(sample_dir / f"sample_{idx:02d}.png", dpi=300)
#         plt.close(fig)

#         # bar plot comparing amplitudes
#         bar_fig, bar_ax = plt.subplots(figsize=(6, 3))
#         indices = np.arange(num_detector)
#         width = 0.35
#         bar_ax.bar(indices - width / 2, amp_label_norm, width, label="Label")
#         bar_ax.bar(indices + width / 2, amp_pred_norm, width, label="Model output")
#         bar_ax.set_xlabel("Spot index")
#         bar_ax.set_ylabel("Amplitude")
#         bar_ax.set_ylim(0.0, 1.05)
#         bar_ax.set_xticks(indices)
#         bar_ax.legend(loc="upper right")
#         bar_ax.grid(axis="y", linestyle="--", alpha=0.4)
#         bar_fig.tight_layout()
#         bar_fig.savefig(sample_dir / f"sample_{idx:02d}_bars.png", dpi=300)
#         plt.close(bar_fig)

#         savemat(
#             sample_dir / f"sample_{idx:02d}.mat",
#             {
#                 "input_raw_complex": raw_padded.astype(np.complex64, copy=False),
#                 "input_phase0": phase0_field.astype(np.complex64, copy=False),
#                 "model_output": pred_map.astype(np.float32, copy=False),
#                 "label": label_map.astype(np.float32, copy=False),
#                 "difference": diff_map.astype(np.float32, copy=False),
#                 "pred_amplitudes": amp_pred.astype(np.float32, copy=False),
#                 "pred_amplitudes_norm": amp_pred_norm.astype(np.float32, copy=False),
#                 "pred_amplitudes_share": amp_pred_share.astype(np.float32, copy=False),
#                 "label_amplitudes": amp_label.astype(np.float32, copy=False),
#                 "label_amplitudes_norm": amp_label_norm.astype(np.float32, copy=False),
#                 "label_amplitudes_share": amp_label_share.astype(np.float32, copy=False),
#             },
#         )
#     print(f"Saved sample visualizations and data to {sample_dir.resolve()}")
