...
# ==========================================================
@torch.no_grad()
def forward_full_intensity(model, images):
    """
    Run the model in eval mode and return:
        I_crop : (B, 1, H_out, W_out) intensity restricted to the inner
                 (un-padded) region — kept identical to ``I_big`` because
                 the model already returns the canvas-aligned intensity.
        I_big  : (B, 1, H_out, W_out) full intensity on the out_size canvas.

    The previous implementation re-implemented Propagation manually,
    which broke after D2NNModel got an out_size canvas.  Now we just
    ...
    try:
        ...
        # Single-WL: model returns (B, 1, out, out)
        # Multi-WL : model returns (B, L, out, out)
        I = model(images)
        if I.ndim == 4 and I.shape[1] > 1:
            # If the caller is on the single-WL evaluation path but the
            # model is multi-WL, take the base wavelength channel.
            base_idx = (model.propagation.wavelengths.numel() // 2
                        if hasattr(model.propagation, "wavelengths") else 0)
            I = I[:, base_idx:base_idx + 1].contiguous()

        I = I.to(torch.float32)
        return I, I                                       # I_crop == I_big now
    finally:
        if was_training:
            model.train()
