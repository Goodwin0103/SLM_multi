...
class ModelGeometry:
    ...
    z_prop: float
    pixel_size: float
    wavelength: float                                  # legacy single-wl field (always set)
    z_input_to_first: float
    padding_ratio: float = 0.5
    out_size: Optional[int] = None
    padding_ratio_out: Optional[float] = None
    # ----- NEW: multi-wl unified entry -----
    wavelengths: Optional[Tuple[float, ...]] = None
    base_wavelength_idx: Optional[int] = None

    @property
    def wavelength_list(self) -> Tuple[float, ...]:
        """Always return a non-empty tuple of wavelengths."""
        if self.wavelengths is not None and len(self.wavelengths) > 0:
            return tuple(self.wavelengths)
        return (self.wavelength,)

    @property
    def is_multiwavelength(self) -> bool:
        return len(self.wavelength_list) > 1
...
# ---------------------------------------------------------------------
# Single-wavelength helpers (existing, kept verbatim)
# ---------------------------------------------------------------------
def scale_phase_masks_for_wavelength(
    ...
    return [np.mod(mask * scale, 2.0 * np.pi) for mask in phase_masks]
...
def _infer_target_region(label_map: np.ndarray, evaluation_regions) -> int:
    scores = [float(label_map[y0:y1, x0:x1].mean()) for (x0, x1, y0, y1) in evaluation_regions]
    return int(np.argmax(scores))


def compute_relative_amp_error_wavelength_sweep(
    phase_masks, *, base_wavelength, wavelength_list, geometry, device,
    test_loader, evaluation_regions, detect_radius, pred_case, num_modes,
    phase_option, amplitudes, amplitudes_phases, phases, mmf_modes,
    field_size, image_test_data,
):
    errors, metrics_list = [], []
    ...
    for lam in wavelength_array:
        ...
        metrics = evaluate_spot_metrics(
            ...
            detect_radius=detect_radius, device=device,
            pred_case=pred_case, num_modes=num_modes, phase_option=phase_option,
            amplitudes=amplitudes, amplitudes_phases=amplitudes_phases, phases=phases,
            mmf_modes=mmf_modes, field_size=field_size, image_test_data=image_test_data,
        )
...
def compute_mode_isolation_wavelength_sweep(
    phase_masks, *, base_wavelength, wavelength_list, geometry, device,
    test_loader, evaluation_regions, detect_radius,
):
    wavelength_array = np.asarray(list(wavelength_list), dtype=np.float64)
    isolation_ratio_fraction_rows, detection_fraction_rows = [], []
        ...
        for images, labels in test_loader:
            ...
            for sample_idx in range(outputs_np.shape[0]):
                ...
                energies, ratios = spot_energy_ratios_circle(
                    intensity, evaluation_regions, detect_radius, offset=(0, 0), eps=1e-12,
                )
                ...
                det_rows_this.append(detection_fractions.astype(np.float64))
                ratios_this.append(float(detection_fractions[target_idx]))
        isolation_ratio_fraction_rows.append(np.asarray(ratios_this, dtype=np.float64))
        detection_fraction_rows.append(np.vstack(det_rows_this))
        if device.type == "cuda":
            torch.cuda.empty_cache()
    ...
    return {
        ...
        "isolation_ratio_mean": isolation_ratios_percent.mean(axis=1),
