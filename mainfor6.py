...
# if torch.backends.mps.is_available():         #优先用 Mac的Apple MPS
...
# 变波长： Wavelength sweep selection: "mode_isolation", "relative_amp_error", or "auto" to match evaluation_mode
...
num_layer_option = [3]   # Define the different layer-number ODNN
...
# parse_known_args lets this work even when run inside Sper/VSCode cells
def _parse_frontend_args():
    p = argparse.ArgumentParser(add_help=False)
    p.add_argument("--config",   type=str, default=None)
    p.add_argument("--mat_file", type=str, default=None)
    args, _ = p.parse_known_args()
    return args

_cli = _parse_frontend_args()
_cfg: dict = {}
if _cli.config:
    try:
        ...
        print(f"Warning: could not load config file '{_cli.config}': {_e}")

if _cfg:
    layer_size            = int(_cfg.get("layer_size",            layer_size))
    layer_size_options    = [layer_size]
    num_modes             = int(_cfg.get("num_modes",             num_modes))
    num_modes_sweep_options = [num_modes]
    field_size            = int(_cfg.get("field_size",            field_size))
    batch_size            = int(_cfg.get("batch_size",            batch_size))
    wavelength            = float(_cfg.get("wavelength_nm",       wavelength * 1e9)) * 1e-9
...
# these are used inside the training loop; keep as None when no config provided
_epochs_cfg    = int(_cfg.get("epochs",        100))  if _cfg else None
_lr_cfg        = float(_cfg.get("learning_rate", 1.99)) if _cfg else None
_lr_gamma_cfg  = float(_cfg.get("lr_gamma",      0.99)) if _cfg else None

# path to the metrics log written during training
_METRICS_LOG = Path(__file__).parent / "frontend" / "logs" / "metrics.jsonl"


#%% 从 .mat 载入 (H, W, M) 的复数模场
max_modes_needed = max([num_modes] + num_modes_sweep_options)
_mat_file_path = _cli.mat_file if _cli.mat_file else 'mmf_6modes_25_PD_1.15.mat'
eigenmodes_OM4 = load_complex_modes_from_mat(
    _mat_file_path,
    key='modes_field'
)
print("Loaded modes shape:", eigenmodes_OM4.shape, "dtype:", eigenmodes_OM4.dtype)
# auto-detect field_size from .mat spatial dimension if not overridden via config
if "field_size" not in _cfg:
    field_size = eigenmodes_OM4.shape[0]
    print(f"Auto-detected field_size={field_size} from .mat file")
...
#%% 考虑多种layersize的可能，D2NN models and train them，
...
for num_layer in num_layer_option:
    ...
    _lr    = _lr_cfg       if _lr_cfg       is not None else 1.99
    _gamma = _lr_gamma_cfg if _lr_gamma_cfg is not None else 0.99
    epochs = _epochs_cfg   if _epochs_cfg   is not None else 1000   #训练次数
    optimizer = optim.Adam(D2NN.parameters(), lr=_lr)
    scheduler = ExponentialLR(optimizer, gamma=_gamma)
    losses = []
    epoch_durations: list[float] = []
    training_start_time = time.time()

    # clear metrics log for this run
    ...
    for epoch in range(1, epochs + 1):
        ...
        # write metrics for frontend monitoring
        _elapsed = time.time() - training_start_time
        _etr     = (_elapsed / epoch) * (epochs - epoch)
        _cur_lr  = optimizer.param_groups[0]['lr']
        with open(_METRICS_LOG, "a") as _mf:
            _mf.write(json.dumps({
                "epoch":        epoch,
                "loss":         round(avg_loss, 8),
                "elapsed_time": round(_elapsed, 2),
                "etr":          round(_etr, 2),
                "lr":           round(_cur_lr, 10),
            }) + "\n")
...
#%% Wavelength sweep: per-mode isolation for each trained layer count
if (
    pred_case == 1
    ...
    for layer_idx, num_layer in enumerate(num_layer_option):
        wl_tag = datetime.now().strftime("%Y%m%d_%H%M%S")
        phase_masks = all_phase_masks[layer_idx]
        ...
        ax_wl.set_title(f"Per-mode isolation vs. wavelength (m{num_modes}_ls{layer_size}_L{num_layer})")
        ax_wl.grid(True, alpha=0.3)
        ax_wl.legend(title="Modes", ncol=2)

        file_stem = f"per_mode_isolation_m{num_modes}_ls{layer_size}_L{num_layer}_{wl_tag}"
        wl_plot_path = wl_dir / f"{file_stem}.png"
        fig_wl.savefig(wl_plot_path, dpi=300, bbox_inches="tight")
        plt.close(fig_wl)

        wl_mat_path = wl_dir / f"{file_stem}.mat"
        savemat(
            str(wl_mat_path),
            {
        ...
        print(f"✔ Per-mode isolation (m{num_modes}_ls{layer_size}_L{num_layer}) plot -> {wl_plot_path}")
        print(f"✔ Per-mode isolation (m{num_modes}_ls{layer_size}_L{num_layer}) data -> {wl_mat_path}")
