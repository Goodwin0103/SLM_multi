...
from ODNN_functions import generate_complex_weights,generate_fields_ts,create_labels,create_evaluation_regions
import os, csv, math
...
MMF_data = MMF_data * np.exp(1j * np.angle(MMF_data))
...
amplitudes_phases_test = amplitudes_phases.copy()
phases_test = phases.copy()
...
image_test_data_raw = generate_fields_ts(complex_weights_ts,MMF_data_ts,num_data,num_modes,field_size)
image_data = image_test_data_raw
...
use_same_dataset_for_test = False

if use_same_dataset_for_test:
    num_test_data = len(train_tensor_data)
    image_test_data = image_data[:num_test_data, :, :, :]
    image_test_data_raw = image_data_raw[:num_test_data, :, :, :]
    label_test_data = label_data[:num_test_data, :, :, :]
    amplitudes_phases_test = amplitudes_phases[:num_test_data].copy()
    phases_test = phases[:num_test_data].copy()
    ...
    if phase_option in (3, 5):
        phase_option_test = phase_option
        ...
        MMF_Label_test_data = torch.zeros([layer_size, layer_size, num_modes])
        for index in range(num_modes):
            MMF_Label_test_data[:, :, index] = torch.from_numpy(
                create_labels(layer_size, layer_size, num_modes, focus_radius, index + 1)
            )
        for index in range(num_test_data):
            target_weights = torch.from_numpy(amplitudes_phases_test[index, :num_modes]).float()
            label_test_data[index, :, :, :] = (target_weights * MMF_Label_test_data).sum(dim=2)
        ...
        test_loader = DataLoader(
            test_tensor_data,
            batch_size=len(image_test_data),
            shuffle=False,
            # num_workers=0,
            # pin_memory=pin_mem,
            # persistent_workers=False 
    ...
    if phase_option == 4: # need to generation the random test dataset
        ...
        MMF_data_ts = torch.from_numpy(MMF_data)
        complex_weights_test_ts = torch.from_numpy(complex_weights_test)
        image_test_data_raw = generate_fields_ts(complex_weights_test_ts,MMF_data_ts,num_test_data,num_modes,field_size) # images for test dataset generation
        image_test_data = image_test_data_raw
        ...
        for index in range(num_test_data):
            target_weights = torch.from_numpy(amplitudes_phases_test[index, :num_modes]).float()
            label_test_data[index, :, :, :] = (target_weights * MMF_Label_test_data).sum(dim=2)  # labels for test dataset generation
...
if num_visual > 0:
    ...
    raw_small_np = image_test_data_raw[:num_visual, 0].detach().cpu().numpy()
