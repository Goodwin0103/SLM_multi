...
SYNTHETIC_CONFIG = {
    ...
    "amplitude_weights": np.array(
        [0.3062, 0.4336, 0.2755, 0.1975,0.7359, 0.2486], dtype=np.float32
    ),
    "phase_option": 3,
    "phase_seed": 20251024,
...
def prepare_synthetic_inputs(config: dict) -> dict:
    ...
    amp = np.asarray(config["amplitude_weights"], dtype=np.float32)
    if amp.size != num_modes:
    ...
    phase_sets = config.get("phase_sets")
    if phase_sets is not None:
        phases = np.asarray(phase_sets, dtype=np.float32)
        if phases.ndim != 2 or phases.shape[1] != num_modes:
            raise ValueError("phase_sets 需要是 shape=(num_samples, num_modes) 的二维数组")
        num_samples = phases.shape[0]
    else:
        num_samples = int(config["num_samples"])
        rng = np.random.default_rng(config.get("phase_seed"))
        phases = sample_phase_weights(num_samples, num_modes, int(config["phase_option"]), rng)

    complex_weights = amp[None, :] * np.exp(1j * phases)
...
amp_weights = synthetic_inputs["amplitude_weights"]
...
amp_df = pd.DataFrame(
    {
        ...
        "amplitude_weight": amp_weights.astype(np.float32),
    }
)
amp_df.to_csv(output_dir / "synthetic_amplitude_weights.csv", index=False)
np.save(output_dir / "synthetic_amplitude_weights.npy", amp_weights.astype(np.float32))
...
for idx, sample_tag in enumerate(sample_tags):
    ...
    weights_per_sample.append((sample_tag, weights, weights_sq, energies))
    ...
    weights_per_sample_phase0.append(
        (sample_tag, weights_phase0, weights_sq_phase0, energies_phase0)
    )
    print(f"✔ {sample_tag} weights (phase=0): {np.round(weights_phase0, 4)}")

if weights_per_sample:
    records = []
    for tag, weights, weights_sq, energies in weights_per_sample:
        row = {"sample_tag": tag}
        for idx_mode in range(num_modes):
            row[f"target_amp_{idx_mode + 1}"] = float(amp_weights[idx_mode])
...
if weights_per_sample_phase0:
    records_phase0 = []
    for tag, weights, weights_sq, energies in weights_per_sample_phase0:
        row = {"sample_tag": tag}
        for idx_mode in range(num_modes):
            row[f"target_amp_{idx_mode + 1}"] = float(amp_weights[idx_mode])
            row[f"weight_{idx_mode + 1}"] = float(weights[idx_mode])
...
if camera_outputs:
    ...
    weights_by_tag = {tag: weights for tag, weights, _, _ in weights_per_sample}
    weights_sq_by_tag = {tag: weights_sq for tag, _, weights_sq, _ in weights_per_sample}
    ...
    panel_fig, panel_axes = plt.subplots(
        3,
        len(sample_tags),
        figsize=(4 * len(sample_tags), 9),
        squeeze=False,
    )

    for col, tag in enumerate(sample_tags):
        ax_amp = panel_axes[0, col]
        ax_out = panel_axes[1, col]
        ax_bar = panel_axes[2, col]
        ...
        ax_out.set_title(f"Camera output ({tag})")
        ax_out.axis("off")

        bar_positions = np.arange(1, num_modes + 1)
        ax_bar.bar(bar_positions, weights, color="tab:blue")
    ...
    panel_fig.tight_layout(h_pad=0.6)
    ...
    sm_out.set_array([])
    panel_fig.colorbar(
        sm_out, ax=panel_axes[1], fraction=0.046, pad=0.04, orientation="vertical"
    )
    ...
    if camera_outputs_phase0:
        ...
        weights_by_tag_phase0 = {
            tag: weights for tag, weights, _, _ in weights_per_sample_phase0
        }
        weights_sq_by_tag_phase0 = {
            tag: weights_sq for tag, _, weights_sq, _ in weights_per_sample_phase0
        }
        ...
        for col, tag in enumerate(sample_tags):
            ...
            ax_bar_p0 = panel_phase0_axes[2, col]
        ...
        panel_phase0_fig.tight_layout(h_pad=0.6)
